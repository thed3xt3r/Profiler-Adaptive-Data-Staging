...
class TrainingPlotCallback(pl.Callback):
    ...
    def on_train_end(self, trainer, pl_module):
        ...
        fig.suptitle("Training Curves — DeepLabV3Plus Xception", fontsize=14)
        plt.tight_layout()
...
def profile_model(model, dataloader, num_batches=5, device="cuda",
    ...
    with open(report_path, "w") as f:
        f.write("=" * 80 + "\n")
        f.write("PROFILER REPORT — DeepLabV3Plus Xception\n")
        f.write("=" * 80 + "\n\n")
...
def run_training(config, train_loader, val_loader, val_dataset, trial=None):
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename=f"deeplab-xception-trial{trial.number if trial else '0'}-{{epoch:02d}}-{{valid/iou:.4f}}",
        monitor="valid/iou",
        mode="max",
        save_top_k=1,
        save_last=True,
...
def main():
    ...
    PATH_LOG = os.path.join(PROJECT_ROOT, "checkpoints_deeplab_xception")
    ...
    os.makedirs(os.path.join(PROJECT_ROOT, "deeplab_xception", "logs"), exist_ok=True)
    ...
    if args.tune:
        ...
        study = optuna.create_study(direction="maximize", study_name="deeplab_xception_tuning")
