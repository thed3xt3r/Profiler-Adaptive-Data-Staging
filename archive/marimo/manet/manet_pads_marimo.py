...
def _(mo):
    mo.md(
        r"""
        # PADS — MA-Net (EfficientNet-B3) on bing_1k

        Self-contained marimo port of `PADS/manet`: policy-aware data loading
...
@app.cell
def _(A, A_pytorch, BytesIO, Dataset, Image, np, os, shutil, tarfile, tempfile, time):
    ...
    return ArcheoDataset, StagingArcheoDataset, TarArcheoDataset, create_dataset, get_transforms, load_dataset


@app.cell
def _(Dataset, Image, download_drive_file, np, os, resolve_bing1k_drive_folders, time):
...
@app.cell
def _(Accuracy, JaccardIndex, pl, smp, torch):
    class ArcheoModel(pl.LightningModule):
        """Lightning module for archaeological site segmentation (manet)."""
...
def _(
    ...
    def run_training(config, train_loader, val_loader, val_dataset, trial=None):
        ...
        trainer = pl.Trainer(
            ...
            # "warn" (not True): MA-Net's decoder uses bilinear upsampling,
...
def _(mo):
    ...
    ui_checkpoint_dir = mo.ui.text(value="./checkpoints_manet", label="Checkpoint output directory", full_width=True)
...
def _(base_config, optuna, os, run_training, train_loader, val_dataset, val_loader):
    if base_config["mode"] == "standard":
        ...
        _study = optuna.create_study(direction="maximize", study_name="manet_tuning")
        _study.optimize(_objective, n_trials=base_config["tune_trials"])
