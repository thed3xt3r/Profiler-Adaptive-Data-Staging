...
def _(mo):
    mo.md(
        r"""
        # PADS — DeepLabV3Plus (ResNet-50) on bing_1k

        Self-contained marimo port of `PADS/deeplab`: policy-aware data loading
...
def _(
    ...
    def run_training(config, train_loader, val_loader, val_dataset, trial=None):
        ...
        trainer = pl.Trainer(
            ...
            # "warn" (not True): DeepLabV3+'s decoder uses bilinear upsampling,
...
def _(mo):
    ...
    ui_checkpoint_dir = mo.ui.text(value="./checkpoints_deeplab", label="Checkpoint output directory", full_width=True)
...
def _(base_config, optuna, os, run_training, train_loader, val_dataset, val_loader):
    if base_config["mode"] == "standard":
        ...
        _study = optuna.create_study(direction="maximize", study_name="deeplab_tuning")
