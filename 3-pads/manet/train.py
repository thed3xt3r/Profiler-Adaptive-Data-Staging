...
def resolve_project_root(cli_value=None):
    ...
    level above the code (~/Thesis/manet/logs for ~/Thesis/3-pads/manet/train.py),
...
def profile_model(model, dataloader, num_batches=5, device="cuda",
    ...
    with open(report_path, "w") as f:
        f.write("=" * 80 + "\n")
        f.write("PROFILER REPORT — MA-Net EfficientNet-B3\n")
        f.write("=" * 80 + "\n\n")
...
def parse_args():
    ...
    # Measured peak on a 4 GB card at fp16: batch 2 -> 1.41 GB, batch 4 ->
    # 2.56 GB, batch 8 -> 4.88 GB (over capacity; the WDDM driver only
    # "survives" that by spilling to system RAM over PCIe). 4 x 8 keeps the
    # effective batch at 32, matching the HPC runs.
    ...
    # bf16, NOT fp16. EfficientNet-B3 overflows fp16's narrow exponent range
    # (SiLU + squeeze-excite): measured here, train loss went NaN at epoch 4 and
    # every metric collapsed to 0 while EarlyStopping burned its full patience on
    # a dead model. bf16 has fp32's exponent range, is native on Ampere and
    # later, and trains cleanly past that point. DeepLab's ResNet-50 is fp16-safe
    # and keeps 16-mixed.
    parser.add_argument("--precision", type=str, default="32",
                        help="Lightning precision, e.g. bf16-mixed, 16-mixed, 32")
...
def run_training(config, train_loader, val_loader, val_dataset, trial=None):
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename=f"manet-effb3-trial{trial.number if trial else '0'}-epoch{{epoch:02d}}-iou{{valid/iou:.4f}}",
    ...
    trainer = pl.Trainer(
        ...
        # "warn" instead of True: a few cuDNN kernels used here have no
        # deterministic implementation, and strict mode makes them hard errors.
        deterministic="warn",
    )
    ...
    # Resume from this run's own last checkpoint if one already exists (e.g. a
    # previous attempt at this exact trial/config died partway) instead of
    # silently restarting from epoch 0 and losing that progress. --fresh opts
    # out: without it, re-running a config whose last.ckpt already reached
    # max_epochs looks like a no-op ("max_epochs reached", nothing saved).
...
def main():
    ...
    PATH_LOG = os.path.join(PROJECT_ROOT, "checkpoints_manet")
    ...
    print("=" * 60)
    print("MA-Net - MA-Net + EfficientNet-B3 + Focal Loss")
    ...
    os.makedirs(os.path.join(PROJECT_ROOT, "manet", "logs"), exist_ok=True)
    ...
    if base_config.get("full_prestage"):
        ...
        policy_summary = profile_data_pipeline(
            ...
            profile_dir=os.path.join(PROJECT_ROOT, "manet", "logs"),
    ...
    if policy == "stage" and not base_config.get("full_prestage"):
        if base_config.get("prefetch_depth") is not None:
            ...
            depth_dir = os.path.join(PROJECT_ROOT, "manet", "logs")
    ...
    if args.profile_only:
        ...
        fsop_dir = os.path.join(PROJECT_ROOT, "manet", "logs")
    ...
    if args.tune:
        ...
        # Persist the study to disk so an interrupted run (OOM, reboot, Ctrl-C)
        # resumes instead of losing every completed trial and restarting trial
        # numbering — and therefore checkpoint directories — from zero.
        optuna_storage = f"sqlite:///{os.path.join(PATH_LOG, 'optuna_study.db')}"
        study = optuna.create_study(
            direction="maximize",
            study_name="manet_tuning",
