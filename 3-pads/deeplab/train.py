...
def resolve_project_root(cli_value=None):
    ...
    level above the code (~/Thesis/deeplab/logs for ~/Thesis/3-pads/deeplab/train.py),
...
def parse_args():
    ...
    # Measured peak on a 4 GB card at fp16: batch 4 -> 1.76 GB, batch 8 ->
    # 3.16 GB (too tight once validation and the profiler are added).
    # 4 x 8 keeps the effective batch at 32, matching the HPC runs.
...
def run_training(config, train_loader, val_loader, val_dataset, trial=None):
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename=f"deeplab-resnet50-trial{trial.number if trial else '0'}-epoch{{epoch:02d}}-iou{{valid/iou:.4f}}",
...
def main():
    ...
    print("=" * 60)
    print("DeepLab - DeepLabV3Plus + ResNet-50 + Focal Loss")
