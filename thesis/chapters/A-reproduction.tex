...
The discrepancy is attributable to GPU memory.
...
semantics (Appendix~\ref{app:defects}, Table~\ref{tab:accuracy-defects}).
...
Figure~\ref{fig:prediction-examples}, shown in Section~\ref{ch:introduction},
gives the qualitative view for these checkpoints on held-out tiles. This section
states how those examples were chosen and how they sit relative to the rest of
the test set.

\paragraph{Where the models succeed.}
Scoring all 466 site-bearing test tiles under all three checkpoints, roughly a
third score above $0.5$ and the best-performing tenth reach $0.78$--$0.81$. On
these tiles the predicted contour tracks the annotated boundary closely rather
than merely overlapping its extent, and the three architectures agree with one
another to within a few points. This is a real capability: on a substantial
minority of sites the models delineate the mound accurately enough to be
directly useful to the survey workflow the reference study describes.

\paragraph{Where they fail, and how the examples were selected.}
The distribution is bimodal rather than centred: alongside the successes above,
$48$--$57\%$ of site-bearing tiles score below $0.1$, and median per-tile IoU is
$0.165$ (MA-Net), $0.053$ (DeepLabV3+) and $0.000$ (SegFormer). The models tend
either to find a site and outline it well or to miss it almost entirely.
Figure~\ref{fig:prediction-examples}'s examples are therefore selected rather
than randomly sampled -- three from the upper mode, one characteristic failure,
one negative -- because a random draw of five tiles would sample mostly from the
lower mode and represent the successful regime no better than the failing one.

The failure mode is systematic and is the one Section~\ref{sec:repro-error}
quantifies. Taking the worst of the three architectures per tile, median IoU is
$0.000$ for every site-area band below $12\%$ of the crop and rises only for
larger sites. Small sites carry too little tonal evidence to separate them from
the terrain around them, which is the same reason the original authors excluded
sites below $1{,}000$~m\textsuperscript{2} and report a $+3.9$ point gain from
that filtering.

\paragraph{Negatives.}
On tiles containing no annotated site, all three architectures correctly predict
nothing in the large majority of cases: $108$ of the $123$ negative test tiles
are cleanly rejected by all three, with spurious detections on only $5.7\%$
(DeepLabV3+), $7.3\%$ (MA-Net) and $8.9\%$ (SegFormer). This bears on the metric
discussion of Section~\ref{sec:repro-metric}: correctly-rejected empty tiles
score a perfect $1.0$ under the reference's per-image convention and so
contribute materially to the headline figure, and the fact that they are
genuinely rejected -- rather than scored as perfect by an artefact of averaging
-- is what makes that contribution legitimate.

\paragraph{A note on per-tile figures.}
Evaluation random-crops each tile, following the reference protocol, so a single
tile's score depends on the crop drawn as well as on the model. The values
annotated in Figure~\ref{fig:prediction-examples} are those of the crop shown;
the distributional statistics above, taken over all 466 site-bearing tiles, are
the stable quantity.
This appendix documents the reproduction environment, protocol, metric reduction,
and qualitative error analysis needed to interpret the accuracy guardrail used in
the main chapters.
