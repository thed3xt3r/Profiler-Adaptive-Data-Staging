...
\label{ch:literature}
%% ============================================================================
This chapter reviews three bodies of literature that provide the foundation for this thesis: semantic segmentation architectures (Section~\ref{sec:lit-seg}), deep learning for archaeological prospection (Section~\ref{sec:lit-arch}), and input-pipeline performance in large-scale deep learning (Section~\ref{sec:lit-io}). The first two establish the application and computational workload considered in this thesis, while the third examines the input-pipeline performance problem and establishes the research gap addressed by \pads{}.


\subsection{Semantic segmentation architectures}
\label{sec:lit-seg}

Semantic segmentation assigns a class label to every pixel in an image, allowing both the category and spatial extent of objects to be identified. A major development in modern semantic segmentation was the introduction of fully convolutional encoder--decoder architectures, with U-Net~\cite{Ronneberger15} becoming one of the most widely adopted examples. Its contracting path progressively extracts contextual features, while the expanding path restores spatial resolution to produce pixel-level predictions. Skip connections between corresponding encoder and decoder stages preserve fine-grained spatial information that may otherwise be lost during downsampling. Originally developed for biomedical image segmentation with limited annotated training data, U-Net also became well suited to remote-sensing applications, where obtaining large quantities of accurately labelled imagery can be similarly difficult and expensive.
...
SegFormer the model in which data-loading stalls are most prevelant, since a
...
on a large general dataset such as ImageNet~\cite{Deng09} and fine-tuning on the
specific task.
...
First, Fiorucci et al.~\cite{Fiorucci22} argue that standard detection measures are poorly
...
accuracy inversely related, because a more conservative model
...
This is a property of the dataset, not of any particular model, and it bounds what
...
This section looks into the literature discussing gpu utilisation and optimisation techniques. 
...
GPUs spend a substantial fraction of wall-clock time blocked on fetching and
preprocessing, and the effect worsens as GPUs get faster relative to storage
and host CPUs.
...
The many-small-file problem is a specific and well-known bottleneck on parallel
filesystems.
Metadata cost per file is close to constant regardless of how few bytes are read,
so per-file overhead does not scale well, and a dataset of thousands of small images
...
rather than between High Bandwidth Memory and SRAM.
