...
Semantic segmentation assigns a class label to every image pixel~\cite{Ronneberger15,Chen18}.
The modern formulation builds on fully convolutional encoder--decoder networks,
...
similarly small~\cite{Ronneberger15,Casini23}.
...
This characterisation and its implications for archaeological machine learning
are discussed explicitly by Bickler~\cite{Bickler21}.
...
Svogor et al.~\cite{Svogor22} profile the PyTorch~\cite{Paszke19}
\code{DataLoader} specifically
...
Segmentation architecture is not treated as solved in general.  For this specific
workload, however, the reference study found that architectural choice moves IoU by only
