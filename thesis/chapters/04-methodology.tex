...
\section{Motivation: when the DataLoader becomes the bottleneck}
...
\subsection{Stage 1 --- Warm-up profiling}
...
\subsection{Stage 2 --- Bottleneck classification}
...
\subsection{Stage 3 --- Policy selection}
...
\subsection{Stage 4 --- Predictive staging}
...
\begin{itemize}
  ...
  \item \textbf{RQ4} is answered by 5 vs 3, and by a sweep of fixed depths in the
    ablation of Chapter~\ref{ch:ablation} --- the auto-tuned depth should land at or
...
which Chapter~\ref{ch:ablation} probes by synthetically constraining scratch
capacity.
...
Each configuration is repeated \PLACEHOLDER{number of repeats} times and reported as
mean $\pm$ standard deviation, because measurements on a shared filesystem are
...
possible scheduled on the same node class to control for hardware variation.
