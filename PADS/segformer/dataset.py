...
def get_transforms():
    ...
    # CenterCrop, not RandomCrop: validation must be deterministic, otherwise
    # every epoch scores a different random 512x512 patch of each 1024x1024
    # image and val/IoU jitters for reasons unrelated to the model.
...
def _finalize(image, mask, transform):
    """Apply the augmentation pipeline and convert to the model's expected layout:
    float32 image normalised to [0, 1] and transposed to CHW, mask as (1, H, W)."""
    if transform:
