<#
.SYNOPSIS
    Local Windows/NVIDIA launcher for the PADS MA-Net run.

.DESCRIPTION
    The Windows counterpart to PAD_manet.slurm. There is no scheduler and no
...
param(
    ...
    [int]$BatchSize = 8,
    [int]$Accumulate = 4,
    # bf16, not fp16: EfficientNet-B3 overflows fp16 and goes NaN at ~epoch 4.
    [string]$Precision = "bf16-mixed",
    [int]$NumWorkers = 4,
    [int]$TuneTrials = 10,
    [switch]$Fresh,
...
$LogDir   = Join-Path $OutRoot "manet\logs"
...
Write-Output "============================================================"
Write-Output "MA-Net Training - MA-Net + EfficientNet-B3 + Focal Loss"
