...
# model.py re-exports ArcheoModel from 3-pads/deeplab/model.py and, as a side
# effect, adds 3-pads/deeplab to sys.path -- reuse that for GPUUtilMonitor too
...
class TrainingPlotCallback(pl.Callback):
    ...
    def on_train_end(self, trainer, pl_module):
        ...
        fig.suptitle("Training Curves — DeepLabV3+ WebDataset", fontsize=14)
        plt.tight_layout()
...
def profile_model(model, dataloader, num_batches=5, device="cuda",
    ...
    with open(report_path, "w") as f:
        f.write("=" * 80 + "\n")
        f.write("PROFILER REPORT — DeepLabV3+ WebDataset\n")
        f.write("=" * 80 + "\n\n")
...
def run_training(config, train_loader, val_loader, val_dataset_func, trial=None):
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename=f"deeplab-resnet50-wds-trial{trial.number if trial else '0'}-{{epoch:02d}}-{{valid/iou:.4f}}",
        monitor="valid/iou",
        mode="max",
        save_top_k=1,
        save_last=True,
...
def main():
    ...
    PATH_LOG = os.path.join(PROJECT_ROOT, "checkpoints_baseline_wds_deeplab")
    ...
    os.makedirs(os.path.join(PROJECT_ROOT, "2-webdataset", "deeplab", "logs"), exist_ok=True)
    ...
    if args.tune:
        ...
        study = optuna.create_study(direction="maximize", study_name="deeplab_wds_tuning")
