...
# model.py re-exports ArcheoModel from 3-pads/segformer/model.py and, as a side
# effect, adds 3-pads/segformer to sys.path -- reuse that for GPUUtilMonitor too
# rather than duplicating the path-resolution logic a second time.
...
class TrainingPlotCallback(pl.Callback):
    ...
    def on_train_end(self, trainer, pl_module):
        ...
        fig.suptitle("Training Curves — SegFormer WebDataset", fontsize=14)
        plt.tight_layout()
...
def profile_model(model, dataloader, num_batches=5, device="cuda",
    ...
    with open(report_path, "w") as f:
        f.write("=" * 80 + "\n")
        f.write("PROFILER REPORT — SegFormer WebDataset\n")
        f.write("=" * 80 + "\n\n")
...
def profile_fsops(dataset, batch_size=32, num_batches=12, profile_dir="profiler_logs"):
    ...
    the WebDataset pipeline, for the same RQ2 comparison 3-pads/*/train.py's
...
def parse_args():
    ...
    parser.add_argument("--focal_alpha", type=float, default=0.75,
                        help="Focal loss alpha (SegFormer default matches the reference, 0.75).")
...
def run_training(config, train_loader, val_loader, val_dataset_func, trial=None):
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename=f"segformer-{config['encoder']}-wds-trial{trial.number if trial else '0'}-epoch{{epoch:02d}}-iou{{valid/iou:.4f}}",
        monitor="valid/iou",
        mode="max",
        save_top_k=1,
        save_last=True,
...
def main():
    ...
    PATH_LOG = os.path.join(PROJECT_ROOT, "checkpoints_baseline_wds_segformer")
    SHARDS_ROOT = os.path.join(PROJECT_ROOT, "2-webdataset/shards")
    ...
    os.makedirs(os.path.join(PROJECT_ROOT, "2-webdataset", "segformer", "logs"), exist_ok=True)
    ...
    if args.tune:
        ...
        study = optuna.create_study(direction="maximize", study_name="segformer_wds_tuning")
